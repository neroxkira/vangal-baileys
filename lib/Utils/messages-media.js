...
const getImageProcessingLibrary = async () => {
    ...
    const [_jimp, sharp] = await imageProcessingLibraryPromise;
    if (sharp) {
        return { sharp };
    }
...
const getFfmpeg = async () => {
    if (!ffmpegPromise) {
        ffmpegPromise = Promise.resolve().then(() => require('fluent-ffmpeg'));
    }
...
const getHttpStream = async (url, options = {}) => {
    ...
    const { default: axios } = await axiosPromise;
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        var _a, _b;
        const { default: axios } = await (axiosPromise || (axiosPromise = import('axios')));
